...
CREATE TABLE IF NOT EXISTS `bsw_adoption` (
  ...
  `book_id_book` INT UNSIGNED NOT NULL,
  ...
  INDEX `fk_adoption_class1_idx` (`class_id` ASC),
  INDEX `fk_adoption_book1_idx` (`book_id_book` ASC),
  UNIQUE INDEX `adoption_school_year_class_book` (`school_id` ASC, `year_adoption` ASC, `class_id` ASC, `book_id_book` ASC)  COMMENT 'Secondary key (school+year+class+book)\n',
...
-- -----------------------------------------------------
-- Table `bsw_user_has_bsw_school`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bsw_user_has_bsw_school` ;

CREATE TABLE IF NOT EXISTS `bsw_user_has_bsw_school` (
  `bsw_user_id_user` INT UNSIGNED NOT NULL,
  `bsw_school_id_school` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`bsw_user_id_user`, `bsw_school_id_school`),
  INDEX `fk_bsw_user_has_bsw_school_bsw_school1_idx` (`bsw_school_id_school` ASC),
  INDEX `fk_bsw_user_has_bsw_school_bsw_user1_idx` (`bsw_user_id_user` ASC),
  CONSTRAINT `fk_bsw_user_has_bsw_school_bsw_user1`
    FOREIGN KEY (`bsw_user_id_user`)
    REFERENCES `bsw_user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_bsw_user_has_bsw_school_bsw_school1`
    FOREIGN KEY (`bsw_school_id_school`)
    REFERENCES `bsw_school` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
